...
#define MAX_BUFFER_SIZE 256  // parameter for fwrite
    ...
    while (initial == 1) {
        ...
        for (int i = 0; i <= NUM_THREADS; i++) {
            if (first_A_events[i].bM == 1) {
                ...
                int written = snprintf(buffer, MAX_BUFFER_SIZE, "%.3lf  A(%.3lf %.3lf %.3lf %-2d)  D(%d %.3lf %-2d)\n",
                ...
                char buffer[MAX_BUFFER_SIZE];
                int written = snprintf(buffer, MAX_BUFFER_SIZE, "%.3lf  A(%.3lf %.3lf %.3lf %-2d)\n",
                                       first_A_events[i].T, first_A_events[i].V, first_A_events[i].Vth,
            ...
            if (a_event_queue[i].T > Tsim) {
                ...
                if (loc == 1) {
                    for (int j = 1; j < i; j++) {
                        char buffer[MAX_BUFFER_SIZE];
                        int written = snprintf(buffer, MAX_BUFFER_SIZE, "%.3lf  A(%.3lf %.3lf %.3lf %-2d)\n",
                                               a_event_queue[j].T, a_event_queue[j].V, a_event_queue[j].Vth,
                                               a_event_queue[j].dT, a_event_queue[j].bM);
                        if (written > 0) {
                            fwrite(buffer, sizeof(char), (size_t)written, fout);
                        }
                    }
                ...
                char buffer[MAX_BUFFER_SIZE];
                int written = snprintf(buffer, MAX_BUFFER_SIZE, "%.3lf  A(%.3lf %.3lf %.3lf %-2d)  D(%d %.3lf %-2d)\n",
                                       a_event_queue[i].T, a_event_queue[i].V, a_event_queue[i].Vth,
                                       a_event_queue[i].dT, a_event_queue[i].bM, d_event_queue[1].V,
                                       d_event_queue[1].dT, d_event_queue[1].bM);
                if (written > 0) {
                    fwrite(buffer, sizeof(char), (size_t)written, fout);
                }
                ...
                char buffer[MAX_BUFFER_SIZE];
                int written = snprintf(buffer, MAX_BUFFER_SIZE, "%.3lf  A(%.3lf %.3lf %.3lf %-2d)  D(%d %.3lf %-2d)\n",
                                       d_event_queue[2].T, a_event_queue[0].V, a_event_queue[0].Vth,
                                       a_event_queue[0].dT, a_event_queue[0].bM, d_event_queue[2].V,
                                       d_event_queue[2].dT, d_event_queue[2].bM);
                if (written > 0) {
                    fwrite(buffer, sizeof(char), (size_t)written, fout);
                }
                ...
                int written = snprintf(buffer, MAX_BUFFER_SIZE, "%.3lf  A(%.3lf %.3lf %.3lf %-2d)  D(%d %.3lf %-2d)\n",
                                       a_event_queue[i].T, a_event_queue[i].V, a_event_queue[i].Vth,
                                       a_event_queue[i].dT, a_event_queue[i].bM, d_event_queue[0].V,
                                       d_event_queue[0].dT, d_event_queue[0].bM);
                if (written > 0) {
                    fwrite(buffer, sizeof(char), (size_t)written, fout);
                }
